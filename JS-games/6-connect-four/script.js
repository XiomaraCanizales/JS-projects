...
const displyaCurrentPlayer = document.querySelector('#current-player')
let currentPlayer = 1
...
function createBoard() {
    for (let i = 0; i < 49; i++) {
        ...
        grid.appendChild(square).classList.add('square')
        squares.push(square)
    }
    // adding class manually 'cause i don't know other way
    for (let i = 42; i < squares.length; i++) {
    ...
    squares[i].onclick = () => {
        if (squares[i + 7].classList.contains('taken')) {
            if (currentPlayer == 1) {
                ...
                currentPlayer = 2
                displyaCurrentPlayer.innerHTML = 'Player ' + currentPlayer 
